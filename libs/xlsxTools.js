...
exports.generatorExcel = function(filePath, title, data, sheet = 'mySheetName') {
    let arr = arrayTools.spreadArray(data);
    arr.unshift(title);
    var buffer = xlsx.build([{ name: sheet, data: arr }]);
    fsPlus.outputFile(filePath, buffer)
    .catch(console.log)
}
...
exports.parseXlsx = function(filePath, template) {
    ...
    let prevIndex = 0;
    try {
        console.log('开始读取excel....');
        var workSheetsFromBuffers = xlsx.parse(fs.readFileSync(filePath));
        console.log('excel读取完毕....');
        // 遍历多个sheet
        fileDataArr = workSheetsFromBuffers.map((sheet,index) => {
